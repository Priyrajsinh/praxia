...
export const metadata: Metadata = {
  ...
  description:
    "Stage 2 of the Praxia map — build models that predict and explain. Full curriculum: bias-variance, regularization, trees and boosting, clustering, evaluation, uncertainty, interpretability, and the end-to-end prediction project.",
};
...
const meta = validateStageMeta({
  ...
  stageNumber: 2,
  slug: "data-scientist",
  timeRange: "4–8 months from a Data Analyst base",
...
const topics = validateTopics([
  {
    concept: "Problem framing — supervised, unsupervised, and reinforcement",
    whyItMatters:
  ...
  {
    concept: "Regularisation — ridge (L2), lasso (L1), elastic net",
    whyItMatters:
  ...
  {
    concept: "Gradient boosting — XGBoost and LightGBM",
    whyItMatters:
  ...
  {
    concept: "Clustering — k-means, hierarchical, DBSCAN",
    whyItMatters:
  ...
  {
    concept: "Dimensionality reduction — PCA, t-SNE, UMAP",
    whyItMatters:
  ...
  {
    concept: "Model evaluation — ROC/AUC, calibration, precision/recall",
    whyItMatters:
  ...
  {
    concept: "Interpretability — SHAP, permutation importance",
    whyItMatters:
  ...
  {
    concept:
      "Causal inference introduction — potential outcomes, selection bias",
    whyItMatters:
  ...
  {
    concept:
      "Time series basics — stationarity, autocorrelation, decomposition",
    whyItMatters:
    ...
    <article>
      ...
      <StageSection id="what-it-does" title="What this role actually does">
        <p>
          ...
          uncertain those predictions are. Not to explore data — that was Stage
          1. Not to serve models at scale — that is Stage 3. The distinct skill
          here is the full modelling cycle, done rigorously.
        </p>

        <p>
          Day-to-day: a problem framing conversation in the morning, feature
          engineering and EDA through the early afternoon, model comparison and
          cross-validation by 3pm, a SHAP waterfall chart explaining the biggest
          prediction to a stakeholder by 4. The skill is not any single
          algorithm. It is knowing which model assumptions hold for this
          dataset, evaluating without leakage, and explaining the result without
          misrepresentation.
        </p>

        <Marginalia>
          The most common data science mistake is not a wrong algorithm — it is
          a wrong evaluation. A model with leaked features wins every benchmark
          and fails every deployment. Evaluation discipline is the job.
        </Marginalia>

        <p>
          <strong>Where this role sits relative to its neighbours.</strong> The
          data scientist is not a data analyst. You are building predictive and
          generative models, not answering &ldquo;what happened&rdquo; questions
          with statistical tests. The inference mindset of Stage 1 — p-values,
          confidence intervals, hypothesis tests — does not go away; it is
          subsumed. Every model evaluation is a statistical question. Every
          uncertainty interval is inference. Stage 2 uses the same mathematical
          machinery as Stage 1, applied to a different problem.
        </p>
        ...
        <p>
          ...
          The MLE is responsible for its reliability and scale in production. In
          small teams these roles merge; in mature ML organisations they do not.
          This stage ends at the point where you hand off a well-specified,
          well-evaluated model to engineering — not at the point where it is
          serving traffic.
        </p>
        ...
        <p>
          ...
          contaminating the model — future data, target-correlated preprocessing
          fitted on the full dataset, labels leaking into features. It is the
          reason models that look incredible in development fail silently in
          production. Everything in this stage is downstream of understanding
          why leakage happens and how to prevent it.
        </p>
      </StageSection>
      ...
      <StageSection id="mathematics" title="Mathematics required">
        ...
        <p>
          <strong>Linear algebra — the geometry of data.</strong> Vectors as
          ...
          measure of alignment. Rank as the dimensionality of the column space.
          Eigenvalues and eigenvectors at the conceptual level: the directions a
          transformation stretches, and by how much. You need these for PCA, for
          understanding gradient descent on the loss surface, and for reading
          any ML paper that uses matrix notation (all of them).
        </p>

        <p>
          <strong>Calculus — the mathematics of change.</strong> Derivatives as
          rates of change. Partial derivatives: how a function changes as one
          input varies while others are held fixed. The gradient: the vector of
          partial derivatives, pointing in the direction of steepest ascent. The
          chain rule: how derivatives compose through layers of functions — this
          is what backpropagation computes. You do not need to prove the chain
          rule; you need to apply it fluently to composite functions.
        </p>

        <p>
          <strong>
            Probability and statistics — carried forward from Stage 1, extended.
          </strong>{" "}
        ...
        <p>
          <strong>Optimisation — why gradient descent works.</strong> A loss
          ...
          step size — too large and you oscillate or diverge, too small and you
          crawl. The intuition for why this finds a minimum for convex losses
          and a local minimum for non-convex ones. Stochastic gradient descent:
          using a noisy estimate of the gradient computed on a mini-batch, which
          is often faster and sometimes regularises the solution.
        </p>

        <Marginalia>
          The most important thing to understand about gradient descent is that
          it finds a local minimum, not necessarily a global one. For convex
          losses, these are the same. For everything else, the architecture and
          initialisation matter enormously — and that is a Stage 3 topic.
        </Marginalia>

        <h3>Research-grade — where the algorithms become legible</h3>

        <p>
          <strong>Derive the bias-variance decomposition.</strong> The expected
          test error of a model decomposes into irreducible noise, bias squared
          (how far the average model prediction is from the truth), and variance
          (how much the model varies across different training sets). This
          derivation is not hard — it is an expectation calculation using the
          definition of variance — but it makes the tradeoff precise and
          explains why averaging models (bagging) reduces variance without
          increasing bias.
        </p>
        ...
        <p>
          ...
          lie on the axes — where many parameters are exactly zero. This
          geometric picture explains why L1 gives sparse solutions and L2 does
          not. It is one of the most illuminating geometric arguments in all of
          applied mathematics, and it takes about twenty minutes to understand
          once you have the picture in front of you.
        </p>

        <p>
          <strong>Information theory — entropy and KL divergence.</strong>{" "}
          Shannon entropy H(X) = −Σ p(x) log p(x) measures the average
          uncertainty in a random variable. The KL divergence D_KL(P ‖ Q)
          measures how much information is lost when Q is used to approximate P.
          These appear everywhere: cross-entropy loss is KL divergence to a
          one-hot distribution; information gain in decision trees is a
          difference in entropy; variational inference minimises KL divergence.
          One chapter of any information theory textbook is sufficient.
        </p>

        <p>
          <strong>SVD and its connection to PCA.</strong> The singular value
          decomposition A = UΣVᵀ decomposes any matrix into rotations and
          scaling. PCA is SVD applied to the centred data matrix. The principal
          components are the right singular vectors; the explained variance is
          the squared singular values. Understanding this makes PCA a tool
          rather than a black box, and it opens the door to the matrix
        ...
        <p>
          ...
          special case of a principled probabilistic algorithm, and explains why
      ...
      <StageSection id="tools" title="Tools and engineering skills">
        <p>
          <strong>scikit-learn — deeply, not just the fit/predict API.</strong>{" "}
          ...
          <code>cross_val_score</code> and <code>StratifiedKFold</code> —
          understanding why you pass the full unfitted pipeline, not a
          pre-fitted transformer. <code>GridSearchCV</code> and{" "}
          <code>RandomizedSearchCV</code> for hyperparameter search with nested
          cross-validation when the dataset is small. The{" "}
          <code>calibration_curve</code> utility for checking whether your
          model&rsquo;s predicted probabilities are meaningful.
        </p>

        <p>
          <strong>XGBoost and LightGBM.</strong> Both are gradient boosted trees
          with different implementation strategies — LightGBM uses leaf-wise
          growth and histogram binning, making it faster on large datasets;
          XGBoost uses level-wise growth and is often more stable. In practice:
          try both, tune the learning rate and number of rounds carefully (these
          interact), and use early stopping on a validation set. Understand the
          regularisation parameters (<code>lambda</code>, <code>alpha</code>,{" "}
          <code>min_child_weight</code>) — they are not knobs to turn until the
          model improves; they are controls on the bias-variance tradeoff with
        ...
        <p>
          ...
          practical implementation that is fast enough for production. Know the
          difference between global explanations (feature importance plots,
          beeswarm plots) and local explanations (waterfall plots, force plots
          for a single prediction). Know when SHAP explanations are misleading:
          highly correlated features split the attribution between them in ways
        ...
        <p>
          ...
          discipline of logging every run — parameters, metrics, artifact paths
          — so you can reproduce any previous result is the difference between a
          scientist and a notebook tinkerer. Pick one, use it from the first
          experiment of every project, and never run an unlogged experiment
          again.
        </p>

        <p>
          <strong>Conformal prediction libraries.</strong> The modern standard
          for uncertainty quantification requires minimal code — the{" "}
        ...
        <p>
          <strong>Pandas and NumPy at scale.</strong> Not just the basics —
      ...
      <StageSection id="project" title="The project">
        <p>
          Three projects — an end-to-end prediction system, a feature
          engineering deep-dive, and a paper reproduction — together prove the
          full Stage 2 competency. The first shows you can build and evaluate
          correctly. The second shows you can squeeze performance from data
          rather than just from model choice. The third shows you can engage
          with the research literature and understand what reproducible results
          look like.
        </p>

        <p className="text-sm text-faded-ink">
          Expected total time: 12–18 weeks alongside the curriculum. Build them
          in order — each one uses skills developed in the previous.
        </p>
        ...
        <p>
          ...
          predictive model — correctly evaluated, properly calibrated,
        ...
        <p>
          ...
          sources: Kaggle, the UCI ML Repository, government open data portals.
        </p>

        <p>
          Execute the full pipeline in order, using a scikit-learn Pipeline that
          wraps every step from raw features to final predictions:
        </p>

        <ol>
          <li>
            <strong>Problem framing and baseline.</strong> State the prediction
            task formally: what is the input, what is the output, what is the
            evaluation metric, and why. Build a baseline — a dummy classifier or
            the mean prediction for regression — and record its performance.
            Every model you build for the rest of the project must beat the
            baseline or it is not progress.
          </li>
          <li>
            <strong>EDA and leakage audit.</strong> For every feature, ask
            whether it can be known at prediction time in production. A feature
            computed from future data, a feature that directly encodes the
            target, or a feature that is only available after the event you are
            predicting — all are leaks. Document your audit. List every feature
          ...
          <li>
            ...
            the hypothesis behind each: &ldquo;users who contacted support more
            than twice in the 30 days before their renewal date churn at 3× the
            base rate — so I am creating a binary flag for this.&rdquo; Test
            whether each feature improves performance in isolation before adding
            it to the full model.
          </li>
          <li>
            ...
            inside each fold — never fit a preprocessor on the full dataset
            before splitting. Report mean ± standard deviation of your primary
            metric across folds. The standard deviation tells you whether your
            result is stable.
          </li>
          <li>
            <strong>Calibration.</strong> For classification: plot the
            calibration curve of your best model. Is the predicted probability
            of 0.7 associated with about 70% of positive outcomes? If not, apply
            Platt scaling or isotonic regression calibration and re-plot. A
            model whose probabilities are not calibrated should not be used to
            make decisions that depend on those probabilities (most real
            decisions do).
          </li>
          <li>
            <strong>Conformal prediction intervals.</strong> Wrap your best
            model in a conformal predictor (MAPIE or equivalent). Set a coverage
            level of 90%. Report what fraction of held-out test examples fall
            within the predicted interval — it should be close to 90%. This is
            the first place you will encounter what &ldquo;coverage-guaranteed
            uncertainty&rdquo; actually means in practice.
          </li>
          <li>
            <strong>Interpretability.</strong> Compute SHAP values for your best
            model. Produce: (1) a global feature importance beeswarm plot, (2) a
            local explanation for the three most interesting individual
            predictions (the highest-confidence correct, the highest-confidence
            wrong, and the most uncertain). For each local explanation, write
            one sentence describing what the model &ldquo;saw&rdquo; in the data
        ...
        <p>
          ...
          A 10–15 page document: problem definition, data description (including
          the leakage audit), methods (every preprocessing and modelling step),
          results (all cross-validation numbers in a table, calibration plots,
          conformal coverage result), interpretation (SHAP plots + prose
          explanation), limitations (what the model cannot do and why), and a
          one-page executive summary written for a non-technical stakeholder.
          The notebook is the appendix. The report is the deliverable.
        </p>
        ...
        <Marginalia>
          ...
          you expect — because your model is genuinely that uncertain. That
        ...
        <p>
          ...
          more than models, and you can engineer them systematically rather than
          by intuition alone.
        </p>

        <p>
          Take a Kaggle competition dataset (any structured competition, active
          or historical) with a public leaderboard. Start with a baseline: raw
          features, minimal preprocessing, XGBoost with default hyperparameters.
          Record the cross-validation score and leaderboard position. Then, over
          two weeks, engineer features systematically:
        </p>
        ...
        <p>
          The deliverable is a notebook documenting every feature you tried, the
          hypothesis behind it, and whether it improved performance. Expected
          outcome: a 5–15% improvement over baseline from features alone, before
          any hyperparameter tuning. Write a half-page reflection: which
          features helped most, which surprised you, and what you would try with
          more time.
        </p>
        ...
        <p>
          ...
          XGBoost paper (Chen &amp; Guestrin, 2016 — linked in the Resources
          section above) on any of its public benchmarks. Alternatively, find a
          recent tabular-ML paper on Papers With Code that reports results on a
          UCI or Kaggle dataset you can download. The criterion is simple: the
          paper must describe its experimental setup clearly enough that you can
          reproduce it from scratch.
        </p>

        <ol>
          <li>
            <strong>Reproduce the number.</strong> Implement or use the library
            implementation, reproduce the exact dataset split and preprocessing
            the paper describes, and run the evaluation. How close did you get
            to the reported number? Any discrepancy is worth investigating: is
            it a random seed difference, a preprocessing difference, a library
            version difference?
          </li>
          <li>
            ...
            tuned point without showing the sensitivity landscape — this step
            reveals whether the reported result is robust or fragile.
          </li>
          <li>
            <strong>Write a reproduction report.</strong> One to two pages: what
            you reproduced (or failed to reproduce and why), what the
            sensitivity analysis showed, and one observation that surprised you.
            This is the foundation of the skill of reading research papers
            critically — not accepting numbers, but checking them.
          </li>
        </ol>

        {/* ── Extensions ── */}
        <h3>Senior extension — model monitoring design</h3>
        <p>
          ...
          for implementation. This is the bridge to Stage 3 thinking: &ldquo;my
          model is not done when it is trained; it is done when it is
          maintained.&rdquo;
        </p>

        <h3>Research extension — novel evaluation</h3>
        <p>
          Identify a failure mode of your Project 1 model that is not captured
          by your primary metric — a subgroup where performance is
      ...
      <StageSection id="resources" title="Resources">
        <p className="mb-6 text-faded-ink">
          Start with <strong>ISLR</strong> — it is the canonical entry, free,
          and available in both R and Python editions. Pair it with{" "}
          ...
          brother — consult it when ISLR&rsquo;s answer is &ldquo;it&rsquo;s
          more complicated than that.&rdquo; The mathematics resources
          (3Blue1Brown, MIT 18.06, Stat 110) cover the linear algebra, calculus,
          and probability this stage requires — use them in parallel with ISLR,
          not after.
        </p>
      ...
      <StageSection id="done" title="How you know you&rsquo;re done">
        ...
        <ul>
          <li>
            ...
            XGBoost — and justify the choice based on the number of samples, the
            presence of interactions, the interpretability requirement, and the
          ...
          <li>
            ...
            pipeline, and explain why fitting the preprocessor inside each fold
            matters.
          </li>
          <li>
            You can explain what a SHAP value is — not just how to compute it,
            but what it means (the marginal contribution of a feature value,
            averaged over all possible feature orderings) — and describe a
            scenario where the SHAP explanation could be misleading.
          </li>
          <li>
            You can compute a conformal prediction interval and state its
            coverage guarantee precisely: &ldquo;with probability at least 1 − α
            over the calibration set, the interval contains the true
          ...
          <li>
            ...
            set — how confident are you in this number?&rdquo; You can walk
            through: class imbalance, the evaluation protocol (single split vs.
            cross-validation), the possibility of leakage, calibration, and
            whether accuracy is even the right metric for this task.
          </li>
          <li>
            You understand the difference between correlation and causation at
            the operational level: you can give a concrete example from your own
            project where a predictive feature is not a causal factor, and
            explain the implication for any intervention based on that feature.
          </li>
        </ul>
        ...
        <ol>
          <li>
            ...
            less than 0.5%. Explain what likely happened. How would you properly
            evaluate whether Feature A is genuinely important?
          </li>
          <li>
            Your logistic regression classifier has ROC-AUC = 0.91 on the test
            set. Your stakeholder asks: &ldquo;If the model outputs a
            probability of 0.7 for a customer, does that mean there is really a
            70% chance they will churn?&rdquo; How do you answer? What would you
            check?
          </li>
          <li>
          ...
          <li>
            ...
            has filed. Should you include this feature? What question would you
            ask to determine whether it is a leak?
          </li>
          <li>
      ...
      <StageSection id="next-stage" title="Bridge to the next stage">
        <p>
          ...
          for months? These are different questions, and they require different
          skills.
        </p>
        ...
        <p>
          ...
          requests per second, with monitoring, retraining pipelines, and a team
          of engineers depending on it.&rdquo; The mathematics of Stage 2
          continues — you need it for deep learning — but it is joined by
        ...
        <p>
          Deep learning enters properly at Stage 3. The neural network is not a
          mystery if you understand the chain rule, gradient descent, and the
          building blocks from Stage 2. It is a very large, very expressive
          model trained by a very general optimisation algorithm. The concepts
          are the same; the scale and the failure modes are different.
        </p>
        ...
        <p>
          ...
          bias-variance tradeoff. The research branch is available now — if
          Stage 3 production work is not your direction, Stage 2 is the point to
          consider branching toward research.
        </p>
